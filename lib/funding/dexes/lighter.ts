import { ArbiesAssets, AssetValues } from "@/lib/funding/assets"
import { HTTPParams } from "@/app/api/req-params"
import { AssetAndFdg, annualize8HourlyFunding } from "@/app/api/funding/utils"
import type { Dex } from "@/lib/funding/dexes/arbies"
import { NextRequest, NextResponse } from "next/server"

// match Lighter pairs with the local registry ; they are mapped by ids on the api
export const LighterPairRegistry: Record<string, AssetValues> = {
  "SEI": ArbiesAssets.SEI,
  "2Z": ArbiesAssets["2Z"],
  "AAVE": ArbiesAssets.AAVE,
  "FOGO": ArbiesAssets.FOGO,
  "ASML": ArbiesAssets.ASML,
  "NMR": ArbiesAssets.NMR,
  "IWM": ArbiesAssets.IWM,
  "AVNT": ArbiesAssets.AVNT,
  "DASH": ArbiesAssets.DASH,
  "AAPL": ArbiesAssets.AAPL,
  "PROVE": ArbiesAssets.PROVE,
  "CRO": ArbiesAssets.CRO,
  "RIVER": ArbiesAssets.RIVER,
  "ARC": ArbiesAssets.ARC,
  "SPX": ArbiesAssets.SPX6900,
  "WTI": ArbiesAssets.CL,
  "MON": ArbiesAssets.MON,
  "SKR": ArbiesAssets.SKR,
  "PENDLE": ArbiesAssets.PENDLE,
  "BCH": ArbiesAssets.BCH,
  // "KRCOMP": ArbiesAssets.KRCOMP, is this even listed elsewhere?
  "DIA": ArbiesAssets.DIA,
  "EDGE": ArbiesAssets.EDGE,
  "WLD": ArbiesAssets.WLD,
  "MAGS": ArbiesAssets.MAGS,
  "SAMSUNGUSD": ArbiesAssets.SAMSUNG,
  "OP": ArbiesAssets.OP,
  "YZY": ArbiesAssets.YZY,
  "USDCAD": ArbiesAssets.USDCAD,
  "MET": ArbiesAssets.MET,
  "STABLE": ArbiesAssets.STABLE,
  "0G": ArbiesAssets["0G"],
  "COIN": ArbiesAssets.COIN,
  "HYPE": ArbiesAssets.HYPE,
  "STRK": ArbiesAssets.STRK,
  "AMZN": ArbiesAssets.AMZN,
  "SKY": ArbiesAssets.SKY,
  "USDJPY": ArbiesAssets.USDJPY,
  "NATGAS": ArbiesAssets.NATGAS,
  "DOT": ArbiesAssets.DOT,
  "USDCHF": ArbiesAssets.USDCHF,
  "DUSK": ArbiesAssets.DUSK,
  "AERO": ArbiesAssets.AERO,
  "MYX": ArbiesAssets.MYX,
  "XAG": ArbiesAssets.XAG,
  "AVAX": ArbiesAssets.AVAX,
  "SKHYNIXUSD": ArbiesAssets.SKHYNIX,
  "BRENTOIL": ArbiesAssets.BRENTOIL,
  "LIT": ArbiesAssets.LIT,
  "S": ArbiesAssets.S,
  "SNDK": ArbiesAssets.SNDK,
  "ETH": ArbiesAssets.ETH,
  "PLTR": ArbiesAssets.PLTR,
  "WLFI": ArbiesAssets.WLFI,
  "ASTER": ArbiesAssets.ASTER,
  "ONDO": ArbiesAssets.ONDO,
  "ZK": ArbiesAssets.ZK,
  "POPCAT": ArbiesAssets.POPCAT,
  "APEX": ArbiesAssets.APEX,
  "TAO": ArbiesAssets.TAO,
  "ADA": ArbiesAssets.ADA,
  "LINK": ArbiesAssets.LINK,
  "GMX": ArbiesAssets.GMX,
  "BMNR": ArbiesAssets.BMNR,
  "VIRTUAL": ArbiesAssets.VIRTUAL,
  "BERA": ArbiesAssets.BERA,
  "MSTR": ArbiesAssets.MSTR,
  "STBL": ArbiesAssets.STBL,
  "ENA": ArbiesAssets.ENA,
  "PENGU": ArbiesAssets.PENGU,
  "FARTCOIN": ArbiesAssets.FARTCOIN,
  "ICP": ArbiesAssets.ICP,
  "JUP": ArbiesAssets.JUP,
  "AUDUSD": ArbiesAssets.AUDUSD,
  "GOOGL": ArbiesAssets.GOOGL,
  "TSLA": ArbiesAssets.TSLA,
  "UNI": ArbiesAssets.UNI,
  "TON": ArbiesAssets.TON,
  "DOLO": ArbiesAssets.DOLO,
  "VVV": ArbiesAssets.VVV,
  "RESOLV": ArbiesAssets.RESOLV,
  "TRUMP": ArbiesAssets.TRUMP,
  "PIPPIN": ArbiesAssets.PIPPIN,
  "POL": ArbiesAssets.POL,
  "PAXG": ArbiesAssets.PAXG,
  "AXS": ArbiesAssets.AXS,
  "GBPUSD": ArbiesAssets.GBPUSD,
  "AMD": ArbiesAssets.AMD,
  "ZEC": ArbiesAssets.ZEC,
  "CC": ArbiesAssets.CC,
  "TIA": ArbiesAssets.TIA,
  "GRASS": ArbiesAssets.GRASS,
  "EURUSD": ArbiesAssets.EURUSD,
  "MEGA": ArbiesAssets.MEGA,
  "MNT": ArbiesAssets.MNT,
  "XCU": ArbiesAssets.XCU,
  "MORPHO": ArbiesAssets.MORPHO,
  "XMR": ArbiesAssets.XMR,
  "ROBO": ArbiesAssets.ROBO,
  "NEAR": ArbiesAssets.NEAR,
  "BNB": ArbiesAssets.BNB,
  "LDO": ArbiesAssets.LDO,
  "XLM": ArbiesAssets.XLM,
  "INTC": ArbiesAssets.INTC,
  "PYTH": ArbiesAssets.PYTH,
  "ZORA": ArbiesAssets.ZORA,
  "1000TOSHI": ArbiesAssets["1000TOSHI"],
  "BTC": ArbiesAssets.BTC,
  "1000PEPE": ArbiesAssets["1000PEPE"],
  "1000FLOKI": ArbiesAssets["1000FLOKI"],
  "SUI": ArbiesAssets.SUI,
  "LINEA": ArbiesAssets.LINEA,
  "STRC": ArbiesAssets.STRC,
  "URA": ArbiesAssets.URA,
  "DYDX": ArbiesAssets.DYDX,
  "SPY": ArbiesAssets.SP500,
  "XPL": ArbiesAssets.XPL,
  "FIL": ArbiesAssets.FIL,
  "KAITO": ArbiesAssets.KAITO,
  "DOGE": ArbiesAssets.DOGE,
  "MSFT": ArbiesAssets.MSFT,
  "HBAR": ArbiesAssets.HBAR,
  "SOL": ArbiesAssets.SOL,
  "LTC": ArbiesAssets.LTC,
  "EIGEN": ArbiesAssets.EIGEN,
  "CRCL": ArbiesAssets.CRCL,
  "TRX": ArbiesAssets.TRX,
  "XPT": ArbiesAssets.XPT,
  "BIRB": ArbiesAssets.BIRB,
  "1000BONK": ArbiesAssets["1000BONK"],
  "ZRO": ArbiesAssets.ZRO,
  "FF": ArbiesAssets.FF,
  "BOTZ": ArbiesAssets.BOTZ,
  "XPD": ArbiesAssets.XPD,
  "NVDA": ArbiesAssets.NVDA,
  "META": ArbiesAssets.META,
  "PUMP": ArbiesAssets.PUMP,
  "SYRUP": ArbiesAssets.SYRUP,
  "XRP": ArbiesAssets.XRP,
  "IP": ArbiesAssets.IP,
  "AZTEC": ArbiesAssets.AZTEC,
  "1000SHIB": ArbiesAssets["1000SHIB"],
  "ARB": ArbiesAssets.ARB,
  "CRV": ArbiesAssets.CRV,
  "USDKRW": ArbiesAssets.USDKRW,
  "XAU": ArbiesAssets.XAU,
  "NZDUSD": ArbiesAssets.NZDUSD,
  "HYUNDAIUSD": ArbiesAssets.HYUNDAI,
  "HOOD": ArbiesAssets.HOOD,
  "USELESS": ArbiesAssets.USELESS,
  "JTO": ArbiesAssets.JTO,
  "WIF": ArbiesAssets.WIF,
  "ETHFI": ArbiesAssets.ETHFI,
  "EDEN": ArbiesAssets.EDEN,
  "APT": ArbiesAssets.APT,
  "CHIP": ArbiesAssets.CHIP,
}

const LgtrApiUrl = "https://mainnet.zklighter.elliot.ai/api/v1";

const LgtrApiUrlEndpoints = {
  markets: "/markets",
  fundings: "/fundings",
  fundingRates: "/funding-rates",
};

type LighterData = {
  exchange: string;
  symbol: string;
  rate: number;
};

export const LighterDex = {
  Name: "Lighter",
  PairRegistry: LighterPairRegistry,
  ApiUrl: LgtrApiUrl,
  ApiUrlEndpoints: LgtrApiUrlEndpoints,

  async GetCurrentFunding(request: NextRequest) {
    const { searchParams } = new URL(request.url);
    const pairs = searchParams.get(HTTPParams.assets)?.split(",") || [];

    const res = await fetch(
      LgtrApiUrl + LgtrApiUrlEndpoints.fundingRates,
      {
        method: "GET",
        headers: { accept: "application/json" },
      }
    );
    const data = await res.json();
    if (!data) {
      return NextResponse.json({ error: "No data found" }, { status: 404 });
    }

    const assetsAndFundings: AssetAndFdg[] = data.funding_rates
      .filter((universe: LighterData) =>
        universe.exchange.toLowerCase() === LighterDex.Name.toLowerCase() &&
        pairs.includes(LighterPairRegistry[universe.symbol])
      )
      .map((universe: LighterData) => {
        return {
          name: LighterPairRegistry[universe.symbol],
          funding: annualize8HourlyFunding(universe.rate),
        };
      });

    return NextResponse.json(assetsAndFundings);
  },

  GetHstyFunding() {
    return undefined;
  },
} satisfies Dex
